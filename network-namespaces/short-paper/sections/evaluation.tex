...
Additionally, and in order to conduct the experiments, we make use of \texttt{iPerf3} a network bandwidth benchmarking tool~\cite{iperf3}.
...
The client just tries to saturate the link, sending as many packets as it can, and reports the measured capacity.
...
To prove our hypothesis that the large delay after a restore is due to the application's behaviour rather than our implementation, we set up an experiment in which we perform a sequence of dumps and immediate restores of the same established TCP connection.
...
Lastly, in both Figures we observe that, albeit being the experiments running bare processes with \criu slightly faster, the overhead introduced by \runc is negligible.
